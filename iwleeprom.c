...
unsigned int read_id(const char *device, const char* param)
{
	...
	unsigned int id;
	char path[512];
	snprintf(path, sizeof(path), DEVICES_PATH "/%s/%s", device, param);
	if (!(f = fopen(path, "r")))
		return 0;
	fscanf(f,"%x", &id);
	fclose(f);
...
void map_device(void)
{
	...
	int i;
	unsigned int addr;
	snprintf(path, sizeof(path), DEVICES_PATH "/%s/%s", dev.device, "config");
	if (!(f = fopen(path, "r")))
		return;
	fread(data, 64, 1, f);
	fclose(f);
...
#if defined(__APPLE__) && defined(__MACH__)
unsigned char
get_conf_byte(struct pcidev *d, unsigned int pos)
{
  return d->config[pos];
}

unsigned short
get_conf_word(struct pcidev *d, unsigned int pos)
{
  return d->config[pos] | (d->config[pos+1] << 8);
}

void
get_classid(struct pcidev *d, unsigned short *classid)
{
    if (classid != NULL)
    {
        *classid = get_conf_word(d, PCI_LOOKUP_CLASS);
    }
}
void
get_subid(struct pcidev *d, unsigned short *subvp, unsigned short *subdp)
{
    if (subvp != NULL)
    {
...
void search_card(void)
{
    ...
    do
    {
        ...
        pci_lookup_name(pacc, devbuf, sizeof(devbuf), PCI_LOOKUP_VENDOR | PCI_LOOKUP_DEVICE,
                        pacc->devices->vendor_id, pacc->devices->device_id);

        id.device = (char *)malloc(256);
        strncpy(id.device, devbuf, 256);
        id.ven = pacc->devices->vendor_id;
        id.dev = pacc->devices->device_id;
        id.class = (pacc->devices->device_class >> 8);
        id.pcidev = pacc->devices;
        pci_read_block(pacc->devices, 0, data, 64);
        memcpy(id.config, data, sizeof(id.config));
        ...
        get_classid(&id, (unsigned short *)&id.class);
        get_subid(&id, (unsigned short *)&(id.sven), (unsigned short *)&(id.sdev));
        check_device(&id);
	...
	do {
		...
		strncpy(id.device, dentry->d_name, sizeof(id.device);
		check_device(&id);
		if (debug) {
			printf("    %s: class %04x   id %04x:%04x   subid %04x:%04x",
				id.device,
